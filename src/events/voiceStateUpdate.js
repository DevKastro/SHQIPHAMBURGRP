...
export default {
    ...
    async execute(oldState, newState, client) {
        ...
        async function handleVoiceJoin(client, state, config) { const { channel, member } = state; if (!config.triggerChannels.includes(channel.id)) return; const now = Date.now(); if (channelCreationCooldown.has(cooldownKey)) { const lastCreation = channelCreationCooldown.get(cooldownKey); if (now - lastCreation < VOICE_CREATE_COOLDOWN_MS) return; } const existingTempChannel = Object.keys(config.temporaryChannels || {}).find(tempChannelId => { const tempInfo = config.temporaryChannels[tempChannelId]; return tempInfo && tempInfo.ownerId === member.id; }); if (existingTempChannel) { const tempChannel = state.guild.channels.cache.get(existingTempChannel); if (tempChannel) { try { await member.voice.setChannel(tempChannel); return; } catch (error) { logger.warn(error); } } } if (member.voice.channel?.id !== channel.id) return; channelCreationCooldown.set(cooldownKey, now); trimCooldownMapIfNeeded(); await createTemporaryChannel(client, state, config); }
        async function handleVoiceLeave(client, state, config) { const { channel, member } = state; const tempChannelInfo = await getTemporaryChannelInfo(client, state.guild.id, channel.id); if (!tempChannelInfo) return; if (channel.members.size === 0) { await deleteTemporaryChannel(client, channel, state.guild.id); } else if (tempChannelInfo.ownerId === member.id) { const nextMember = channel.members.first(); if (nextMember) await transferChannelOwnership(client, channel, state.guild.id, nextMember.id); } }
        async function handleVoiceMove(client, oldState, newState, config) { if (oldState.channel) { const tempChannelInfo = await getTemporaryChannelInfo(client, oldState.guild.id, oldState.channel.id); if (tempChannelInfo) { if (oldState.channel.members.size === 0) { await deleteTemporaryChannel(client, oldState.channel, oldState.guild.id); } else if (tempChannelInfo.ownerId === oldState.member.id) { const nextMember = oldState.channel.members.first(); if (nextMember) await transferChannelOwnership(client, oldState.channel, oldState.guild.id, nextMember.id); } } } if (config.triggerChannels.includes(newState.channel.id) && !config.triggerChannels.includes(oldState.channel?.id)) { await handleVoiceJoin(client, newState, config); } }
        async function createTemporaryChannel(client, state, config) { const { channel: triggerChannel, member, guild } = state; try { const me = guild.members.me; if (!me) return; const triggerPermissions = triggerChannel.permissionsFor(me); if (!triggerPermissions?.has([PermissionFlagsBits.ManageChannels, PermissionFlagsBits.MoveMembers, PermissionFlagsBits.Connect])) return; const channelOptions = config.channelOptions?.[triggerChannel.id] || {}; const nameTemplate = channelOptions.nameTemplate || config.channelNameTemplate || "{username}'s Room"; let userLimit = channelOptions.userLimit ?? config.userLimit ?? 0; const bitrate = clampVoiceBitrate(channelOptions.bitrate ?? config.bitrate ?? DEFAULT_VOICE_BITRATE); userLimit = Math.max(0, Math.min(99, userLimit || 0)); const channelName = sanitizeVoiceChannelName(formatChannelName(nameTemplate, { username: member.user.username, userTag: member.user.tag, displayName: member.displayName, guildName: guild.name, channelName: triggerChannel.name })); if (!member.voice?.channel || member.voice.channel.id !== triggerChannel.id) return; const tempChannel = await guild.channels.create({ name: channelName, type: ChannelType.GuildVoice, parent: triggerChannel.parentId, userLimit: userLimit === 0 ? undefined : userLimit, bitrate: bitrate, permissionOverwrites: [{ id: member.id, allow: ['Connect', 'Speak', 'PrioritySpeaker', 'MoveMembers'] }, { id: guild.id, allow: ['Connect', 'Speak'] }] }); await member.voice.setChannel(tempChannel); await registerTemporaryChannel(client, guild.id, tempChannel.id, member.id); } catch (error) { logger.error(error); } }
        async function deleteTemporaryChannel(client, channel, guildId) { try { await channel.delete(); await unregisterTemporaryChannel(client, guildId, channel.id); } catch (error) { logger.error(error); } }
        async function transferChannelOwnership(client, channel, guildId, newOwnerId) { try { await channel.permissionOverwrites.set([{ id: newOwnerId, allow: ['Connect', 'Speak', 'PrioritySpeaker', 'MoveMembers'] }, { id: channel.guild.id, allow: ['Connect', 'Speak'] }]); await registerTemporaryChannel(client, guildId, channel.id, newOwnerId); } catch (error) { logger.error(error); } }
        function clampVoiceBitrate(bitrate) { return Math.max(MIN_VOICE_BITRATE, Math.min(MAX_VOICE_BITRATE, bitrate)); }
        function sanitizeVoiceChannelName(name) { if (!name) return FALLBACK_CHANNEL_NAME; const sanitized = sanitizeInput(name).trim(); if (sanitized.length === 0) return FALLBACK_CHANNEL_NAME; return sanitized.substring(0, MAX_CHANNEL_NAME_LENGTH); }
        function cleanupCooldownEntries() { if (channelCreationCooldown.size > MAX_TRACKED_COOLDOWNS) { const now = Date.now(); for (const [key, value] of channelCreationCooldown.entries()) { if (now - value > VOICE_CREATE_COOLDOWN_MS) channelCreationCooldown.delete(key); } } }
